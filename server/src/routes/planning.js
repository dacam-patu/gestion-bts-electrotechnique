...
// Fonction utilitaire pour obtenir le nom de la phase
const getPhaseName = (phaseNumber) => {
  ...
  return phaseNames[phaseNumber] || 'Phase inconnue';
};
...
router.post('/', (req, res) => {
  ...
  if (!student_id) {
    console.error('❌ student_id manquant:', { student_id, phase, phase_name, start_date, end_date });
  ...
  // Valeurs par défaut
  const finalPhase = parseInt(phase) || 1;
  const finalPhaseName = phase_name || getPhaseName(finalPhase);
  const finalStartDate = start_date || new Date().toISOString().split('T')[0];
  const finalEndDate = end_date || finalStartDate;
  const finalStartTime = start_time || '08:00';
  const finalEndTime = end_time || '17:00';
  
  // Validation de la phase
  if (![1, 2, 3].includes(finalPhase)) {
    return res.status(400).json({ 
      success: false, 
  ...
  console.log('✅ Données validées et corrigées:', {
    student_id,
    project_id,
    phase: finalPhase,
  ...
  const finalColor = color || getDefaultColorForPhase(finalPhase);
